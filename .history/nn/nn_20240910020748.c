...
float* simple_error(float *predicted, float *actual, size_t size){
...
void fc_backward(FCLayer *layer, float *d_output){
    ...
    // compute propogated error
    float *d_input = (float *)calloc(layer->input_size, sizeof(float));
    for (int i = 0; i < layer->output_size; ++i){
        for (int j = 0; j < layer->input_size; ++i){
           d_input[j] += d_output[i] * layer->weights[j * layer->output_size + i];
        }
    }
