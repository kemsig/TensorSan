...
void fc_backward(FCLayer *layer, float *d_output) {
    // Ensure d_output and layer pointers are valid
    if (!layer || !d_output) {
        fprintf(stderr, "Invalid layer or d_output pointer\n");
        exit(EXIT_FAILURE);
    }
    ...
    if (!d_activation) {
        perror("Failed to allocate memory for d_activation");
        exit(EXIT_FAILURE);
    }
    ...
    // Multiply d_output by derivative of activation function
    for (int i = 0; i < layer->output_size; ++i) {
